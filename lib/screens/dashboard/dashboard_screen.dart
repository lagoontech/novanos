...
class DashboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    int pendingLength = dashBoardController.nextClientsList.length >= 5
        ? 5
        : dashBoardController.nextClientsList.length;

    refresh() {
      dashBoardController.getCheckInClients();
      dashBoardController.getNextClient();
      nextVisitList = dashBoardController.nextClientsList;
      checkInClientsList = dashBoardController.checkInClientsList;
      pendingLength = dashBoardController.nextClientsList.length >= 5
          ? 5
          : dashBoardController.nextClientsList.length;
    }
                        ...
                        TextButton(
                            onPressed: () {
                              Get.to(() => MorePendingClients(
                                    pendingClientsList:
                                        dashBoardController.nextClientsList,
                                  ));
                            },
                            child: Row(
                              children: [
                        ...
                        return nextVisitList.isEmpty
                            ...
                            : TileCheckInOut(
                                name: dashBoardController
                                        .nextClientsList[index].customerName ??
                                    'Not Available',
                                address: dashBoardController
                                        .nextClientsList[index].location ??
                                    'Not Available',
                                status: 'VISIT',
                                checkFunc: () async {
                                  await checkIn(dashBoardController
                                      .nextClientsList[index]);
                                },
                              );
                      },
                      separatorBuilder: (context, index) {
